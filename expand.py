...
def Expand():
    ...
    with container_1:
        # st.title("`Novel Writer`")
        st.info("This is an AI novel writer. Please put the subject of your novel here. Thank you!")
        
    ################################################
        
    if "messages_temp_5" not in st.session_state:
        st.session_state.messages_temp_5 = []


    with container_1:
        for message in st.session_state.messages_temp_5:
            with st.chat_message(message["role"]):
                st.markdown(message["content"], unsafe_allow_html = True)

    if prompt := st.chat_input("Enter the subject of your novel!"):
        st.session_state.messages_temp_5.append({"role": str("Novel")+"user", "content": prompt})
        with container_1:
            ...
            with st.chat_message("assistant"):
                ...
                try: 
                    full_response = ""
                    prompt = PromptCls.ExpandingPromptStyle(prompt)
                    ...
                    for i in full_response:
                        ...
                        time.sleep(0.01)
                    message_placeholder.markdown(full_response)
            st.session_state.messages_temp_5.append({"role": "assistant", "content": full_response})
