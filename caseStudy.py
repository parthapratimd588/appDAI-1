...
def CaseStudy():
    ...
    with container_1:
        # st.title("`Case Study`")
        st.info("This is an AI case study writer. Please put your subject here to do case study. Thank you!")
        
    ################################################
        
    if "messages_temp_2" not in st.session_state:
        st.session_state.messages_temp_2 = []

    
    with container_1:
        for message in st.session_state.messages_temp_2:
            with st.chat_message(message["role"]):
                st.markdown(message["content"], unsafe_allow_html = True)

    if prompt := st.chat_input("Put here to do case study"):
        st.session_state.messages_temp_2.append({"role": str("Case")+"user", "content": prompt})
        with container_1:
            ...
            with st.chat_message("assistant"):
                ...
                try: 
                    full_response = ""
                    prompt = PromptCls.CaseStudyPromptStyle(prompt)
                    ...
                    for i in full_response:
                        ...
                        time.sleep(0.01)
                    message_placeholder.markdown(full_response)
            st.session_state.messages_temp_2.append({"role": "assistant", "content": full_response})
