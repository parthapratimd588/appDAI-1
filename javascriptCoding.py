...
def JavaScriptCoding():
    ...
    ################################################
    # st.info("This is an AI Javascript coder. Put your subject of code or code snippet here. Thank you!")
    container_1 = st.container(height = 350, border = False)

    with container_1:
        # st.title("`Javascript Coder`")
        st.info("This is an AI Javascript coder. Put your subject of code or code snippet here. Thank you!")
    
    ################################################
        
    if "messages_temp_10" not in st.session_state:
        st.session_state.messages_temp_10 = []

    
    with container_1:
        for message in st.session_state.messages_temp_10:
            with st.chat_message(message["role"]):
                st.markdown(message["content"], unsafe_allow_html = True)

    if prompt := st.chat_input("Enter the subject!"):
        st.session_state.messages_temp_10.append({"role": str("Python")+"user", "content": prompt})
        with container_1:
            ...
            with st.chat_message("assistant"):
                ...
                try: 
                    full_response = ""
                    prompt = PromptCls.JavaScriptCodingPromptStyle(prompt)
                    ...
                    for i in full_response:
                        ...
                        time.sleep(0.01)
                    message_placeholder.markdown(full_response)
            st.session_state.messages_temp_10.append({"role": "assistant", "content": full_response})
