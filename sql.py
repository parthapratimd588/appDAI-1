...
def SQLs():
    ...
    with container_1:
        # st.title("`SQL Helper`")
        st.info("This is an AI sql solver. Put your subject of code or code snippet here. Thank you!")
    
    ################################################
        
    if "messages_temp_19" not in st.session_state:
        st.session_state.messages_temp_19 = []

    with container_1:
        for message in st.session_state.messages_temp_19:
            with st.chat_message(message["role"]):
                st.markdown(message["content"], unsafe_allow_html = True)

    if prompt := st.chat_input("Enter the subject!"):
        st.session_state.messages_temp_19.append({"role": str("SQL")+"user", "content": prompt})
        with container_1:
            ...
            with st.chat_message("assistant"):
                ...
                try: 
                    full_response = ""
                    prompt = PromptCls.SQLPromptStyle(prompt)
                    ...
                    for i in full_response:
                        ...
                        time.sleep(0.01)
                    message_placeholder.markdown(full_response)
            st.session_state.messages_temp_19.append({"role": "assistant", "content": full_response})
