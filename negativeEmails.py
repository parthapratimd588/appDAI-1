...
def NegativeEmails():
    ...
    with container_1:
        # st.title("`Negative Email Attender`")
        st.info("This is an AI email attender for negative response. Put your subject of email for the negative reaction. Thank you!")
    
    ################################################
        
    if "messages_temp_14" not in st.session_state:
        st.session_state.messages_temp_14 = []



    with container_1:
        for message in st.session_state.messages_temp_14:
            with st.chat_message(message["role"]):
                st.markdown(message["content"], unsafe_allow_html = True)

    if prompt := st.chat_input("Put subject here."):
        st.session_state.messages_temp_14.append({"role": str("Email")+"user", "content": prompt})
        with container_1:
            sentiments = "Negative"
            ...
            with st.chat_message("assistant"):
                ...
                try: 
                    ...
                    for i in full_response:
                        ...
                        time.sleep(0.01)
                    message_placeholder.markdown(full_response)
            st.session_state.messages_temp_14.append({"role": "assistant", "content": full_response})
