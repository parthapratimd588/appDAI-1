...
def CSSCoding():
    ...
    with container_1:
        # st.title("`css Coder`")
        st.info("This is an AI css coder. Put your subject of code or code snippet here. Thank you!")
    
    ################################################
    
    if "messages_temp_3" not in st.session_state:
        st.session_state.messages_temp_3 = []


    with container_1:
        for message in st.session_state.messages_temp_3:
            with st.chat_message(message["role"]):
                st.markdown(message["content"], unsafe_allow_html = True)

    if prompt := st.chat_input("Enter the subject!"):
        st.session_state.messages_temp_3.append({"role": str("Python")+"user", "content": prompt})
        with container_1:
            ...
            with st.chat_message("assistant"):
                ...
                try: 
                    full_response = ""
                    prompt = PromptCls.CSSCodingPromptStyle(prompt)
                    ...
                    for i in full_response:
                        ...
                        time.sleep(0.01)
                    message_placeholder.markdown(full_response)
            st.session_state.messages_temp_3.append({"role": "assistant", "content": full_response})
