...
def Summarize():
    ...
    with container_1:
        # st.title("`Summarizer`")
        st.info("This is an AI summarizer chatbot. Put your informative phrase here. Thank you!")
        
    ################################################
        
    if "messages_temp_21" not in st.session_state:
        st.session_state.messages_temp_21 = []

    with container_1:
        for message in st.session_state.messages_temp_21:
            with st.chat_message(message["role"]):
                st.markdown(message["content"], unsafe_allow_html = True)

    if prompt := st.chat_input("Paste the phrase here!"):
        st.session_state.messages_temp_21.append({"role": str("Summarize")+"user", "content": prompt})
        with container_1:
            ...
            with st.chat_message("assistant"):
                ...
                try: 
                    full_response = ""
                    prompt = PromptCls.SummarizePromptStyle(prompt)
                    ...
                    for i in full_response:
                        ...
                        time.sleep(0.01)
                    message_placeholder.markdown(full_response)
            st.session_state.messages_temp_21.append({"role": "assistant", "content": full_response})
